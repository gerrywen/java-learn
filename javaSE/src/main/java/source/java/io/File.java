    ...
    implements Serializable, Comparable<File>
{
     ...
     *
     * 这个抽象路径名是规范化的路径名字符串。规范化的路径名字符串使用默认的名称分隔符，不包含任何重复或冗余的分隔符。
     *
     * @serial
     */
     ...
     *
     * 检查文件是否有无效的路径。目前，对文件路径的检查是非常有限的，它只包括Nul字符检查。
     * 返回true意味着该路径肯定是无效/垃圾路径。但是返回false并不保证路径是有效的。
     *
     * 如果文件路径无效，则为true。
     */
     ...
     *
     * 已经规范化的路径名字符串的内部构造函数。
     * 参数顺序用于从公共(文件、字符串)构造函数中消除此方法的歧义。
     */
    ...
    public File(String parent, String child) {
        ...
        if (parent != null) {
            if (parent.equals("")) {
                // 根据父路径名解析子路径名字符串。两个字符串都必须是标准形式的，结果也必须是标准形式的。
                this.path = fs.resolve(fs.getDefaultParent(),// 返回父路径名字符串，当两个参数的文件构造函数中的父目录参数为空路径名时使用该字符串。
                                       fs.normalize(child));
            } else {
     ...
     *          is empty
     *
     * @return 由这个抽象路径名表示的文件或目录的名称，如果该路径名的名称序列为空，则为空字符串
     */
...
//     * java.nio.file.Files#readAttributes(Path,Class,LinkOption[])
     ...
     *
     *          删除File对象表示的文件或目录，如果表示的是目录，需要保证目录是空的，否则无法删除。
     ...
     *
     *
     * 可以看出两个方法的区别是，delete是立即执行删除，而deleteOnExit是程序退出虚拟机时才会删除。重点关注deleteOnExit方法
     */
