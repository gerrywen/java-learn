...
public abstract class AbstractExecutorService implements ExecutorService {
    ...
    /**
     * 为给定Runnable任务和默认返回值构造一个RunnableFuture
     */
    ...
    private <T> T doInvokeAny(Collection<? extends Callable<T>> tasks,
        ...
        // 使用ExecutorCompletionService来管理大量任务执行的结果
        // ExecutorCompletionService内部使用LinkedBlockingQueue作为完成队列，任务执行完后自动将结果Future加入到队列中
        // https://my.oschina.net/7001/blog/874638
...
//                ee = new ExecutionException();
            ...
            for (int i = 0, size = futures.size(); i < size; i++)
                futures.get(i).cancel(true);//取消剩余任务
        }
    }
    ...
    /**
     * 执行给定的任务列表，当所有任务完成时才返回保持任务状态和结果的 Future 列表
     * @param tasks
     * @param <T>
     * @return
     * @throws InterruptedException
    ...
    public <T> List<Future<T>> invokeAll(Collection<? extends Callable<T>> tasks)
        ...
        try {
            ...
            if (!done)
                for (int i = 0, size = futures.size(); i < size; i++)
                    futures.get(i).cancel(true);//取消所有正在执行的其他任务
        }
    }
    ...
    public <T> List<Future<T>> invokeAll(Collection<? extends Callable<T>> tasks,
        ...
        try {
            ...
            if (!done)//任务超时或执行异常
                for (int i = 0, size = futures.size(); i < size; i++)
                    futures.get(i).cancel(true);//取消正在执行的任务
        }
    }
